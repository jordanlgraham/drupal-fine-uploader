...
$clientPrivateKey = $serverPrivateKey = '[your_AWS_private_key]';
// These two keys are only needed if the delete file feature is enabled
// or if you are, for example, confirming the file size in a successEndpoint
// handler via S3's SDK, as we are doing in this example.

$serverPublicKey = '[your_AWS_public_key]';//$_SERVER['key'];
//$serverPrivateKey = '[public]';// $_SERVER['PARAM2'];

//error_log(json_encode($_SERVER).'---', 3, "php.log");
// The following variables are used when validating the policy document
// sent by the uploader: 
$expectedBucketName = "[your_bucket_name]";
...
function isPolicyValid($policy) {
    ...
    return true;// $bucket == $expectedBucketName && $parsedMaxSize == (string)$expectedMaxSize;
}
